...
export const initDb = async () => {
  ...
  `);
    ...
    try {
        ...
        // New columns for sync, lock, and multi-device
        ...
        if (!cols.includes('sync_status')) {
            await db.execAsync("ALTER TABLE notes ADD COLUMN sync_status TEXT DEFAULT 'local';");
        }
        if (!cols.includes('device_id')) {
            await db.execAsync('ALTER TABLE notes ADD COLUMN device_id TEXT;');
        }
